...
const AdminDashboard = () => {
  const deleteRecord = async (id: any) => {
    try {
      await axios.delete(`http://185.104.189.135:5280/api/industry/delete?id=${id}`, {
        headers: {
  ...
  const columns: Column[] = [
    ...
    },
    {
      Header: 'Update',
      Cell: ({ row: { original } }) => (
        <Link href={`/admin-dashboard/industries/${original.id}/update`}>
          <svg
  ...
  return (
    <Layout>
      ...
      <div className='flex justify-between items-center mb-2'>
        <h1 className='text-4xl font-bold text-gray-800'>Industries</h1>
        <Link
          href='/admin-dashboard/industries/add'
