...
const AdminDashboard = () => {
  const deleteRecord = async (id: any) => {
    try {
      await axios.delete(`http://185.104.189.135:5280/api/educationfield/delete?id=${id}`, {
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://185.104.189.135:5280/api/educationfield/getall', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
