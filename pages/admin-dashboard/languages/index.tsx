...
const AdminDashboard = () => {
  const deleteRecord = async (id: any) => {
    try {
      await axios.delete(`http://185.104.189.135:5280/api/language/delete?id=${id}`, {
        headers: {
  ...
  const columns: Column[] = [
    ...
    },
    {
      Header: 'Update',
      Cell: ({ row: { original } }) => (
        <Link href={`/admin-dashboard/languages/${original.id}/update`}>
          <svg
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://185.104.189.135:5280/api/language/getall', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  return (
    <Layout>
      ...
      <div className='flex justify-between items-center mb-2'>
        <h1 className='text-4xl font-bold text-gray-800'>Languages</h1>
        <Link
          href='/admin-dashboard/languages/add'
